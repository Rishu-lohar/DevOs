...
const typeConfig = {
  commit: {
    icon: GitBranch,
    color: "text-sky-300 border-sky-500/20 bg-sky-500/10",
  },
  project: {
    icon: FolderKanban,
  ...
  ai: {
    icon: Bot,
    color: "text-pink-300 border-pink-500/20 bg-pink-500/10",
  },
  team: {
    icon: UsersRound,
    ...
    <GlassCard className="divide-y divide-white/[0.06]">
      {activities.map((activity) => {
        ...
        return (
          <div
            key={activity.title}
            className="flex items-center gap-3.5 p-4 transition-colors hover:bg-white/[0.025]"
          >
            <div
            ...
            <span className="shrink-0 rounded-full border border-white/[0.06] bg-white/[0.02] px-2 py-0.5 text-[10px] text-slate-500">
              {activity.time}
            </span>
          </div>
        );
